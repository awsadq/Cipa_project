...
<div class="mb-3">
    <label>Тренер</label>
    <select name="trainer_id" id="trainerSelect" class="form-control" onchange="showTrainerInfo(this)">
        <option value="">—</option>
    ...
    <div id="trainerInfo" class="mt-3" style="display: none;">
        <div class="d-flex align-items-center">
            <img id="trainerPhoto" src="" alt="Фото тренера" class="rounded-circle me-3" style="width: 80px; height: 80px; object-fit: cover;">
            <div>
                <strong id="trainerPosition"></strong><br>
                <small id="trainerBio"></small>
            </div>
        </div>
    </div>
</div>
...
<script>
    function showTrainerInfo(select) {
        const option = select.options[select.selectedIndex];
        const photo = option.dataset.photo;
        const position = option.dataset.position;
        const bio = option.dataset.bio;

        if (photo && position && bio) {
            document.getElementById('trainerPhoto').src = photo;
            document.getElementById('trainerPosition').textContent = position;
            document.getElementById('trainerBio').textContent = bio;
            document.getElementById('trainerInfo').style.display = 'flex';
        } else {
            document.getElementById('trainerInfo').style.display = 'none';
        }
    }

    // автоотображение, если selected сохранён через old()
    window.addEventListener('DOMContentLoaded', function () {
        const select = document.getElementById('trainerSelect');
        if (select && select.value) {
            showTrainerInfo(select);
        }
    });
</script>
