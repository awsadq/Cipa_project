...
@section('content')
    ...
    <div class="news-create-wrapper">
        ...
        <h2 class="news-heading">Редактировать новость</h2>

        <form class="news-form" method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="news-form-group">
                <label class="news-label">Заголовок</label>
                <input type="text" name="title" value="{{ old('title', $news->title) }}" class="news-input @error('title') is-invalid @enderror" required>
                @error('title')
            ...
            <div class="news-form-group">
                ...
                <textarea name="content" class="news-input @error('content') is-invalid @enderror" rows="5" required>{{ old('content', $news->content) }}</textarea>
                @error('content')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="news-form-group">
                <label class="news-label">Текущее изображение</label><br>
                @if($news->image)
                    <img src="{{ asset('storage/' . $news->image) }}" width="200" class="img-thumbnail d-block mb-3" alt="Текущее изображение"><br>
                @endif
                <label class="news-label">Новое изображение</label>
                <input type="file" name="image" class="news-input @error('image') is-invalid @enderror">
                @error('image')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>

            <button type="submit" class="news-submit-btn">💾 Обновить</button>
        </form>
    </div>
@endsection
